...
import { Search, ChefHat, ShoppingCart, X, Clock, Users, Check } from "lucide-react";
        ...
        <div className="overflow-y-auto flex-1 px-6 pb-6 pt-2 space-y-5">
          ...
          <div className="text-center space-y-1">
            ...
            <div className="flex items-center justify-center gap-3 text-xs text-muted-foreground flex-wrap">
              {recipe.prepTime && <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" />{recipe.prepTime} min</span>}
              {recipe.servings && <span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{recipe.servings} {pt ? "porções" : "servings"}</span>}
              {recipe.difficulty && (
          ...
          {/* Description */}
          {recipe.description && (
            <p className="text-sm text-muted-foreground text-center leading-relaxed">{recipe.description}</p>
          )}
          ...
          <div className="px-6 pb-6 pt-3 border-t border-black/[0.04] dark:border-white/[0.06] shrink-0">
            <button onClick={handleAddMissing}
              ...
              {pt ? `Adicionar ${missingIngredients.length} ingrediente${missingIngredients.length > 1 ? "s" : ""} faltante${missingIngredients.length > 1 ? "s" : ""} à lista`
                : es ? `Agregar ${missingIngredients.length} ingrediente${missingIngredients.length > 1 ? "s" : ""}`
