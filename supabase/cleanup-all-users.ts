/**
 * Cleanup All Users Except Test User
 *
...
async function cleanup() {
  ...
  try {
    ...
    const testUserId = testUser?.users.find(
    ...
    log.warning(`${usersToDelete} usuários serão deletados.`);

    if (usersToDelete === 0) {
      log.info("Nenhum usuário para deletar. Finalizando.");
      return;
    }
    ...
    // 2. Deletar dados em cascata usando RPC (SQL direto)
    ...
    log.success(`✅ ${usersToDelete} usuários e seus dados foram deletados.`);
    log.success("✅ Todos os CPFs foram removidos do banco.");
  } catch (error) {
    log.error(`Erro durante limpeza: ${(error as Error).message}`);
    console.error(error);
    process.exit(1);
  }
}

async function deleteUserDataManually(supabase: any, testUserId: string) {
  /**
  ...
  // Deletar perfis (CUIDADO: deleta usuario e seus dados)
  try {
    const { error } = await supabase
      .from("profiles")
      .delete()
